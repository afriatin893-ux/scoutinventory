...
<div class="table-responsive mb-4">
    <table class="table table-bordered align-middle bg-white">
        <thead><tr><th>Barang</th><th style="width:140px;">Jumlah Diajukan</th><th style="width:140px;">Stok Sekarang</th></tr></thead>
        <tbody>
...
@elseif ($peminjaman->status === 'dikembalikan' && $peminjaman->pengembalians->isNotEmpty())
    ...
    <div class="card">
        ...
        <div class="card-body">
            <p><strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($pengembalian->tanggal_pengembalian)->format('d M Y') }}</p>
            ...
            @if ($pengembalian->foto_kondisi)
                <img src="{{ asset('storage/'.$pengembalian->foto_kondisi) }}" style="max-width:240px;" class="rounded border">
            @endif
            @if ($pengembalian->catatan)<p class="mt-2"><strong>Catatan:</strong> {{ $pengembalian->catatan }}</p>@endif
        </div>
    </div>
@endif
@endsection
